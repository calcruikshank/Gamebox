...
namespace Gameboard.Tools
{
    public class ScreenTouchInput : MonoBehaviour
    {
        ...
        private bool TryInputFromScreenPosition(Vector3 inPosition)
        {
            ...
            Vector3 scenePosition = Gameboard.singleton.gameCamera != null ? Gameboard.singleton.gameCamera.ScreenToWorldPoint(inPosition) : Camera.main.ScreenToWorldPoint(inPosition);
            switch (downAxis)
            {
        ...
        {
            Vector3 scenePosition = Camera.main.ScreenToWorldPoint(inCurrentPosition);
            switch (downAxis)
            {
                case AxisAngles.X: scenePosition.x = transform.position.x; break;
                case AxisAngles.Y: scenePosition.y = transform.position.y; break;
